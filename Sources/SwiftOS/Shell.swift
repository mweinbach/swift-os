...
final class Shell {
    ...
    private func dispatch(_ command: String, args: [String], stdin: String?) -> String {
        ...
        case "uname":
            if args.contains("-a") {
                return "SwiftOS \(Kernel.shared.hostname) \(Kernel.shared.kernelRelease) #1 SMP \(Kernel.shared.machine)"
            }
            return "SwiftOS"
        case "whoami": return environment["USER"] ?? "user"
        case "hostname": return Kernel.shared.hostname
        case "ps": return cmdPS()
        case "uptime": return cmdUptime()
        case "date": return Shell.dateFormatter.string(from: Date())
    ...
    private func cmdPS() -> String {
        ...
        for process in Kernel.shared.processes {
            let pid = pad(String(process.pid), to: 5)
            let name = padLeft(process.name, to: 22)
            let cpu = pad(String(format: "%.1f", process.cpuPercent), to: 6)
            let mem = pad(String(format: "%.1f", process.memoryMB), to: 8)
    ...
    private func cmdUptime() -> String {
        let up = Kernel.shared.uptime
        let (one, five, fifteen) = Kernel.shared.loadAverage
        return " \(Shell.timeFormatter.string(from: Date())) up \(uptimeDuration(up)), 1 user, load average: "
            + String(format: "%.2f, %.2f, %.2f", one, five, fifteen)
    }
    ...
    private func cmdFree(_ args: [String]) -> String {
        let total = Kernel.shared.totalMemoryMB
        let used = Kernel.shared.usedMemoryMB
    ...
    private func cmdNeofetch() -> String {
        ...
        let info: [String] = [
            ...
            "Host: Metal GPU",
            "Kernel: \(kernel.kernelRelease)",
            "Uptime: \(friendlyUptime(kernel.uptime))",
            "Shell: \(kernel.shellName)",
            "WM: \(kernel.wmName)",
            "Terminal: \(kernel.terminalName)",
            "CPU: Apple Silicon",
            "Memory: \(Int(kernel.usedMemoryMB))MiB / \(Int(kernel.totalMemoryMB))MiB",
        ]
    ...
    private func copyTree(_ source: String, _ dest: String) throws {
    ...
    private func longFormat(_ node: VNode) -> String {
        pad(node.permissions, to: 11) + pad(String(node.size), to: 7) + " "
            + Shell.longDateFormatter.string(from: node.modified) + " " + node.name
    }
    ...
    private func uptimeDuration(_ interval: TimeInterval) -> String {
        let total = Int(interval)
        let days = total / 86400
        let hours = (total % 86400) / 3600
        let minutes = (total % 3600) / 60
        if days > 0 {
            return "\(days) day\(days == 1 ? "" : "s"), \(hours):" + String(format: "%02d", minutes)
        }
    ...
    private func human(_ megabytes: Double) -> String {
        if megabytes >= 1024 { return String(format: "%.1fG", megabytes / 1024) }
        if megabytes >= 1 { return String(format: "%.0fM", megabytes) }
        return "0"
    }

    /// Formats a VFS error bash-style, but with the path the user actually typed.
    private func vfsErrorMessage(_ argument: String, _ error: Error) -> String {
        guard let vfsError = error as? VFSError else {
            return "\(argument): \(error.localizedDescription)"
        }
        switch vfsError {
