...
final class Desktop {
    ...
    private var cachedWindowButtons: [(id: UUID, rect: CGRect)] = []
    ...
    private init() {
        ...
        clockFormatter.dateFormat = "EEE HH:mm"
    }
    ...
    private func bootMessage(from line: String) -> String {
        if let range = line.range(of: "systemd[1]: ") {
    ...
    private func drawWifi(_ surface: Surface, center: CGPoint) {
        ...
        for radius in radii {
            var previous: CGPoint?
            for step in 0...4 {
                let angle = (200 + Double(step) * 35) * Double.pi / 180
    ...
    private func drawBattery(_ surface: Surface, rect: CGRect) {
        ...
        let level = 0.9 * (1 - Kernel.shared.uptime.truncatingRemainder(dividingBy: 7200) / 7200) + 0.1
    ...
    private func drawTaskbar(_ surface: Surface, size: CGSize) {
        ...
        for window in wm.windows {
            ...
            cachedWindowButtons.append((window.id, rect))
    ...
    private func activate(_ target: Target) {
        ...
        case .windowButton(let id):
            let wm = WindowManager.shared
            guard let window = wm.windows.first(where: { $0.id == id }) else { return }
            if window.isMinimized {
                wm.focus(window)
            } else if wm.focused === window {
                wm.minimize(window)
    ...
    private func updateClock() {
        ...
        clockMinute = minute
        clockString = clockFormatter.string(from: now)
    }
}
