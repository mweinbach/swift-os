...
///
/// Embedded Swift port: OSApp is a base class (override, not protocol conformance),
/// dates are UInt64 wall-clock ms formatted with TimeFmt.fileDate, String(format:)
/// became NumFmt, and the VFS throws typed `VFSError`.
final class FileManagerApp: OSApp {
    ...
    override var title: String { "Files" }
    override var preferredContentSize: CGSize { CGSize(width: 640, height: 430) }
    ...
    /// Case-insensitive comparison standing in for localizedCaseInsensitiveCompare
    /// (Foundation). Returns < 0, 0, > 0; equal-insensitive strings report 0.
    private static func ciCompare(_ a: String, _ b: String) -> Int {
    ...
    private func applySort() {
        func compare(_ a: VNode, _ b: VNode) -> Bool {
            switch sortKey {
            case .name:
                let c = FileManagerApp.ciCompare(a.name, b.name)
                if c == 0 { return a.name < b.name }
                return sortAscending ? c < 0 : c > 0
            case .size:
                if a.size != b.size { return sortAscending ? a.size < b.size : a.size > b.size }
                return FileManagerApp.ciCompare(a.name, b.name) < 0
            case .modified:
    ...
    private static func formatUnit(_ value: Double, _ unit: String) -> String {
        value < 10 ? NumFmt.f1(value) + " " + unit : NumFmt.fixed(value, 0) + " " + unit
    }
    ...
    private func drawSidebar(_ s: Surface, rect: CGRect) {
        ...
        for place in FileManagerApp.places {
            ...
            let selected = place.path == currentPath ||
                (place.path != "/" && currentPath.starts(with: place.path + "/"))
    ...
    private func drawMainList(_ s: Surface, rect: CGRect) {
        ...
        for (i, node) in entries.enumerated() {
            ...
            let dateStr = TimeFmt.fileDate(node.modified)
