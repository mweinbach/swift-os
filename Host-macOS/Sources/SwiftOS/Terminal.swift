...
/// Reads a string from the macOS clipboard. AppKit is used ONLY for this paste
/// bridge; everything inside the OS is rendered through the Surface API.
...
final class TerminalApp: OSApp {
    ...
    private func appendOutput(_ output: String) {
        ...
        var parts = output.replacingOccurrences(of: "\r", with: "")
    ...
    private func handleKey(_ key: KeyEvent) {
        ...
        if mods.contains(.command) {
            if key.keyCode == 9 || key.characters.lowercased() == "v" { pasteFromClipboard() }
            return
        }
    ...
    private func pasteFromClipboard() {
        guard let raw = clipboardString(), !raw.isEmpty else { return }
        // The input line is single-line: flatten newlines, drop control characters.
        let flattened = raw.components(separatedBy: .newlines).joined(separator: " ")
        var cleaned = ""
        for scalar in flattened.unicodeScalars where scalar.value >= 0x20 && scalar.value != 0x7F {
    ...
    private func completeWord() {
        ...
        let isFirstWord = upToCursor[..<wordStart]
        ...
        for candidate in unique.dropFirst() {
            common = commonPrefix(common, candidate)
        }
